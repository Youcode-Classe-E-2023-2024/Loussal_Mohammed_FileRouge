                                        ...
                                        <td>
                                            <div class="d-flex justify-content-left align-items-center">
                                                ...
                                                <div class="d-flex flex-column">
                                                    <span class="text-truncate fw-bold">{{substr($post->body, 0, 15)}}</span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink4">
                                                    ...
                                                    @unless(!is_null($post->deleted_at))
                                                        <form action="{{route('dropPost', ['post' => $post->id])}}" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="dropdown-item text-white hover:text-purple-900 hover:bg-indigo-600">Delete</button>
                                                        </form>
                                                    @else
                                                        <form action="{{route('restorePost', ['post' => $post->id])}}" method="post">
                                                            @csrf
