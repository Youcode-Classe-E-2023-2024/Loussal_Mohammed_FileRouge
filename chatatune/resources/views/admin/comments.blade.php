                                        ...
                                        <td>
                                            <div class="d-flex justify-content-left align-items-center">
                                                ...
                                                <div class="d-flex flex-column">
                                                    <span class="text-truncate fw-bold">{{substr($comment->body, 0, 15)}}</span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink4">
                                                    ...
                                                    @unless(!is_null($comment->deleted_at))
                                                        <form action="{{route('dropPost', ['comment' => $comment->id])}}" method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="dropdown-item text-white hover:text-purple-900 hover:bg-indigo-600">Delete</button>
                                                        </form>
                                                    @else
                                                        <form action="{{route('restorePost', ['comment' => $comment->id])}}" method="post">
                                                            @csrf
